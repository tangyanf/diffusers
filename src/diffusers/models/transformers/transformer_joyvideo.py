...
class JoyVideoModulate(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            raise ValueError("JoyVideoAttnProcessor requires encoder_hidden_states (text stream)")
...
class JoyVideoAttention(nn.Module, AttentionModuleMixin):
    _default_processor_cls = JoyVideoAttnProcessor
    _available_processors = [JoyVideoAttnProcessor]
...
class JoyVideoTransformerBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        num_attention_heads: int,
        ...
        self.img_mod = JoyVideoModulate(dim, factor=6)
        ...
        self.txt_mod = JoyVideoModulate(dim, factor=6)
        ...
        self.attn = JoyVideoAttention(dim, num_attention_heads, attention_head_dim, eps=eps)
...
class JoyVideoTimeTextImageEmbedding(nn.Module):
...
class JoyVideoTransformer3DModel(ModelMixin, ConfigMixin, AttentionMixin):
    _skip_layerwise_casting_patterns = ["img_in", "condition_embedder", "norm"]
    _no_split_modules = ["JoyVideoTransformerBlock"]
    _supports_gradient_checkpointing = True
    _keep_in_fp32_modules = ["time_embedder", "norm1", "norm2", "norm_out"]
    _repeated_blocks = ["JoyVideoTransformerBlock"]
    ...
    def __init__(
        ...
        self.condition_embedder = JoyVideoTimeTextImageEmbedding(
            dim=hidden_size,
            time_freq_dim=256,
    ...
    def get_rotary_pos_embed(self, vis_rope_size: list[int], txt_rope_size: int | None = None):
        target_ndim = 3
